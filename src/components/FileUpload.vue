...
import Compressor from 'compressorjs'; // 导入 compressorjs
...
watch(() => props.modelValue, (newVal) => {
  ...
  if (newVal) {
    ...
    if (Array.isArray(newVal)) {
      files = (newVal as (string | UploadedFile)[]).map(item => {
        const urlString = item && typeof item === 'object' && typeof item.url === 'string' ? item.url : String(item);
        if (urlString) {
          return {
            url: urlString,
            fileName: (item as UploadedFile).fileName || urlString.split('/').pop() || urlString,
            type: (item as UploadedFile).type || (urlString.match(/\.(jpeg|jpg|gif|png|svg|webp)$/i) ? 'image' : 'unknown'),
            filePath: (item as UploadedFile).filePath || '', // 确保 filePath 存在
            recognized_text: (item as UploadedFile).recognized_text || null
          };
        }
        return null;
      ...
      for (const file of filesToUpload) {
        ...
        const compressedFile = await new Promise<File>((resolve, reject) => {
          new Compressor(file, {
            ...
            maxWidth: 1920, // 最大宽度
            maxHeight: 1080, // 最大高度
            mimeType: 'image/jpeg', // 输出 JPEG 格式
            success(result) {
        ...
        const uploadResult = await uploadFile(compressedFile); // 修改这里，接收包含 publicUrl 和 filePath 的对象
